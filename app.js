...
app.get("/admin/tickets/:page", (req, res) => {
    const page = parseInt(req.params.page); // Parse page to integer
    const limit = 3;

    Ticket.paginate({}, { page, limit })
        .then(result => {
            const formattedTickets = {};

            result.docs.forEach(ticket => {
                const formattedTicket = ticket.ticket.map(innerArr => innerArr.join(', '));
                formattedTickets[ticket._id.toString()] = formattedTicket;
            });
            ...
            const response = {
                ...
                total_tickets: result.totalDocs
            };
